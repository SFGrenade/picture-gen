...
static void fft_radix2(MY_COMPLEX * z, unsigned size)
{
    ...
    std::vector<MY_COMPLEX> ww(size / 2);

    for (unsigned i = 0; i < size / 2; ++i)
    {
        ww[i] = std::exp(-2.0 * M_PI * MY_COMPLEX(0.0, 1.0) * MY_COMPLEX_BASE(i) / MY_COMPLEX_BASE(size));
    }
    ...
    while (num_subffts != 0)
    {
        for (unsigned i = 0; i < num_subffts; ++i)
        {
            ...
            for (unsigned j = 0; j < size_subfft / 2; ++j)
            {
                ...
                const MY_COMPLEX w = ww[ww_index];

                const MY_COMPLEX   zleft  =     z[left];
                const MY_COMPLEX w_zright = w * z[right];
...
static void czt(MY_COMPLEX * z, unsigned n, MY_COMPLEX * ztrans, unsigned m, MY_COMPLEX w, MY_COMPLEX a)
{
    ...
    std::vector<MY_COMPLEX> zz(fft_size);
    ...
    for (unsigned k = 0; k < fft_size; ++k)
    {
        if (k < n)
        {
            const MY_COMPLEX w1 = std::pow(w, 0.5 * k * k) / std::pow(a, k);
            zz[k] = w1 * z[k];
        }
        else
        {
    ...
    fft_radix2(zz.data(), fft_size);

    // Allocate and initialize w2 that we will convolve with.

    std::vector<MY_COMPLEX> w2(fft_size);
    ...
    for (unsigned k = 0; k < fft_size; ++k)
    {
        if (k < n + m - 1)
        {
            ...
            w2[k] = std::pow(w, -0.5 * kshift * kshift);
        }
        else
        {
            w2[k] = MY_COMPLEX(0);
    ...
    fft_radix2(zz.data(), fft_size); // forward FFT
    ...
    {
        const MY_COMPLEX w3 = std::pow(w, (0.5 * k * k));
        ztrans[k] = w3 * zz[n - 1 + k];
    }
}

static void czt_fft(MY_COMPLEX * z, unsigned size)
{
    if (size == 0)
    {
        return;
    ...
    else
    {
        ...
        const MY_COMPLEX w = std::exp(-2.0 * M_PI * MY_COMPLEX(0.0, 1.0) / MY_COMPLEX_BASE(size));
        const MY_COMPLEX a = 1;
    ...
    else
    {
        ...
        else
        {
            ...
            for (unsigned i = 0; i < n; ++i)
            {
                ...
                const MY_COMPLEX z = std::exp(M_PI * MY_COMPLEX(0.0, 1.0) * MY_COMPLEX_BASE(i) / MY_COMPLEX_BASE(n));
